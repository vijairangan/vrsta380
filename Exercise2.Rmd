...
We find the author names to use it to attribute the probabilites obtained from naive bayes to their corresponding author.
...
We create a random forest model based on the training documents available for the 50 authors in the training set. The random forest model created is then used to predict the authors of the testing documents. The accuracy of predictions is calculated and the authors that can be easily identified and those that are difficult are obtained below.

Loading the required library. Using the test and the training documentts we create the DTM for these. The two DTM contain 2500 rows. 
...
Retrieving the respective author names and storing as the deoendent variable from the matrix (both test and train)
